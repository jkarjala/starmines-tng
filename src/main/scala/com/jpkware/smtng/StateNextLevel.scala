...
class StateNextLevel(game: Game, options: Map[String,String]) extends State {
    ...
    if (StatePlay.scores.timeBonus>0) {
      ...
      timer.loop(150, () => {
        if (StatePlay.scores.timeBonus>2000)
          ...
          StatePlay.scorebox.addToScore(StatePlay.scores.timeBonus)
  ...
  override def update(): Unit = {
    if (StatePlay.scores.timeBonus>0) return else timeBonusCounted()
  ...
  def gotoPlay(): Unit = {
    if (StatePlay.scores.timeBonus>0) {
      StatePlay.scorebox.addToScore(StatePlay.scores.timeBonus)
      StatePlay.scorebox.addToTimeBonus(-StatePlay.scores.timeBonus)
      timeBonusCounted()
      return
    }
  ...
  def timeBonusCounted(): Unit = {
    ...
    if (!saved) {
      ...
      Progress.updateAndSave(StatePlay.scores, StarMinesNG.debug)
      saved = true
    }
  }
}
